...
async def resolve_pending_episodes():
    ...
    for ep in pending_eps:
        ...
        try:
            ...
            plat_name = detect_platform_name(torrent_title)
            subs_found, audio_found = inspect_media_tracks(v_path)
            final_subs = merge_track_lists("", subs_found, plat_name)
            final_audio = merge_track_lists("", audio_found, plat_name)
            log_message(f"Tracks for {romaji} Ep {ep_num} ({plat_name or 'Default'}): Subs=[{final_subs}] Audio=[{final_audio}]")
            ...
            if not is_cr:
                ...
                await execute_sql("""
                    ...
                    WHERE id = ?
                """, [pd_url, pd_id, pd_url, pd_id, size_mb, stored_source, is_multi_audio, audio_score, final_subs, final_audio, now_str, int(time.time()), pending_until, ep_id])
                ...
                await execute_sql("""
                    ...
                    WHERE id = ?
                """, [pd_url, pd_id, pd_url, pd_id, size_mb, stored_source, is_multi_audio, audio_score, final_subs, final_audio, now_str, int(time.time()), ep_id])
    ...
    for ep in pending:
        ...
        if found_better:
            ...
            try:
                ...
                plat_name = detect_platform_name(found_better["title"])
                subs_found, audio_found = inspect_media_tracks(v_path)
                ...
                log_message(f"Grace: replaced {romaji} Ep {ep_num} with CR Arabic version ({merged_subs})")
            except Exception as e:
    ...
    for ep in recent_eps:
        ...
        if better:
            ...
            try:
                ...
                plat_name = detect_platform_name(target["title"])
                subs_found, audio_found = inspect_media_tracks(v_path)
                merged_subs = merge_track_lists(ep.get("subtitles"), subs_found, plat_name)
                merged_audio = merge_track_lists(ep.get("audio_tracks"), audio_found, plat_name)
